...
$max = new time\ZoneOffset(time\ZoneOffset::MAX_TOTAL_SECONDS);
echo 'Max ZoneOffset = ' . stringify($max) . "\n";

$rnd = new time\ZoneOffset(random_int(time\ZoneOffset::MIN_TOTAL_SECONDS, time\ZoneOffset::MAX_TOTAL_SECONDS));
echo 'Random ZoneOffset = ' . stringify($rnd) . "\n";

echo "> MAX = ";
try {
    new time\ZoneOffset(time\ZoneOffset::MAX_TOTAL_SECONDS + 1);
} catch (Throwable $e) {
    echo $e->getMessage() . "\n";
}
...
echo "< MIN = ";
try {
    new time\ZoneOffset(time\ZoneOffset::MIN_TOTAL_SECONDS - 1);
} catch (Throwable $e) {
    echo $e->getMessage() . "\n";
}
